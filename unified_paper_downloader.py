...
def load_downloaded_dois():
    """Load set of already downloaded DOIs."""
    dois = set()
    
    available = read_csv_safe(AVAILABLE_CSV)
    for paper in available:
        paper = normalize_fieldnames(paper)
...
def try_all_methods(doi, title, email):
    ...
    methods = [
        ...
        ('Sci-Hub', lambda: get_scihub_url(doi), 2.0),
    ]
...
def process_paper(paper_row, total_count, paper_index):
    ...
    try:
        ...
        if doi in downloaded_dois:
            with state_lock:
                processed_count += 1
            log(f"[{processed_count}/{total_count}] SKIP: {title[:40]}... (already downloaded)")
            return
...
def main():
    """Main function."""
    global downloaded_count, failed_count, downloaded_dois, log_file, processed_count
    ...
    log("="*60)
    log("Unified Paper Download Manager")
    log("="*60)
    
    if not os.path.exists(REQUIRED_CSV):
        log(f"ERROR: {REQUIRED_CSV} not found!")
        log(f"Please create {REQUIRED_CSV} with columns: Title, DOI")
        return
    ...
    downloaded_dois = load_downloaded_dois()
    log(f"Already downloaded: {len(downloaded_dois)} papers")
    
    papers = read_csv_safe(REQUIRED_CSV)
    total_count = len(papers)
    
    log(f"Papers in queue: {total_count}")
    log(f"Parallel workers: {MAX_WORKERS}")
    log(f"Download sources: 14 (Unpaywall, ArXiv, Semantic Scholar, OpenAlex, Europe PMC, PubMed, bioRxiv, CORE, DOAJ, BASE, LOC, Publisher, ResearchGate, Sci-Hub)")
    ...
    log("="*60)
    log(f"Total processed: {processed_count}/{total_count}")
    log(f"Successfully downloaded: {downloaded_count}")
    log(f"Failed downloads: {failed_count}")
    log(f"Already existed: {processed_count - downloaded_count - failed_count}")
    if processed_count > 0:
        success_rate = (downloaded_count / processed_count * 100) if processed_count > 0 else 0
